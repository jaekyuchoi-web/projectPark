...
# 업로드 슬롯 정의 (e: 입력 5종)
# 용어 정의 — "전기" = 직전 결산년도(전년도 연간 결산 = 전년도 4분기).
#   결산은 연 1회뿐이므로 당기가 어느 분기든 전기는 항상 직전년도 4분기로 동일하다.
#   예) 당기 2026년 1~4분기 → 전기는 모두 2025년 4분기. '전분기/전반기'는 전기가 아니다.
SLOT_KEYS = [
    "prev_balance",      # 1. 전기 이월 소스 = 직전 결산년도(전년도 4분기) 계정별 잔액 명세서
...
SLOT_LABELS = {
    "prev_balance": "전기 이월 소스 (직전 결산년도=전년도 4분기 계정별 잔액 명세서)",
